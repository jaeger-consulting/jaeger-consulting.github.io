...
require(ggplot2)
require(tidyverse)
data_int <- sampleSizeDuration[, c('maxNumberOfSubjects', 'accrualTime', 'studyDuration')] %>%
...
```
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 5}
p <- ggplot(data = data_int[data_int $ variable %in% c('Futility', 'Power'), ],
            mapping = aes(x = dte,
                          y = value,
                          color = variable,
                          fill = variable)) +
