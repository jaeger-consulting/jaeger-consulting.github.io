...
This blog post will evaluate the impact of the robust exchangeability approach introduced by @Neuenschwander2016 when evaluating the average bioequivalence in a bioequivalence study. 
...
The most commonly used statistical design for comparing a new drug formulation and a reference drug formulation for average bioequivalence is a crossover design with two sequences and two periods, called standard 2x2 crossover design. In this design, each subject is randomized and assigned to a sequence of formulations. Subjects assigned to reference formulation/new formulation (R/N) will receive R in the first dosing period and N in the second dosing period. Subjects assigned to new formulation/reference formulation (N/R) will receive N in the first dosing period and R in the second dosing period. A sufficiently long washout period is imposed between the two dosing periods.
...
- $\delta_{s_{n}}$ and $\epsilon_{n}$ are mutually independent.
...
Supplementary material from @Neuenschwander2016 provides the WinBUGS code for the binary EXNEX model. This can easily be updated to the 2x2 crossover linear model.
