...
Given that PK parameters are (usually) log-normally distributed, log-transformation of exposure measures before statistical analysis is recommended [@FDA2001; @FDA2014]. We are here not discussing how those PK parameters are derived. Please refer to @rosenbaum2016 for a general introduction on pharmacokinetics.
...
For the exchangeability probability $\pi_{i}$, a beta distribution with shape parameters at 3 and 1 is considered. Mode is at 1, but probability of that $\pi_{i}$ is below 0.5 is equal to 0.125, indicating some level of confidence on the exchangeability but also on the non-exchangeability.
